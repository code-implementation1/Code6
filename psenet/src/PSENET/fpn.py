...
class FPN(nn.Cell):
    def __init__(self, in_channels, out_channel, long_size):
        ...
        self._upsample = P.ResizeBilinearV2(align_corners=True)
    ...
    def construct(self, c2, c3, c4, c5):
        ...
        p4 = self._upsample(p5, (self.long_size // 16, self.long_size // 16)) + c4
        p4 = self.smooth_conv_p4(p4)
        p4 = self.smooth_relu_p4(self.smooth_bn_p4(p4))

        c3 = self.reduce_conv_c3(c3)
        c3 = self.reduce_relu_c3(self.reduce_bn_c3(c3))
        p3 = self._upsample(p4, (self.long_size // 8, self.long_size // 8)) + c3
        p3 = self.smooth_conv_p3(p3)
        p3 = self.smooth_relu_p3(self.smooth_bn_p3(p3))

        c2 = self.reduce_conv_c2(c2)
        c2 = self.reduce_relu_c2(self.reduce_bn_c2(c2))
        p2 = self._upsample(p3, (self.long_size // 4, self.long_size // 4)) + c2
        p2 = self.smooth_conv_p2(p2)
        p2 = self.smooth_relu_p2(self.smooth_bn_p2(p2))

        p3 = self._upsample(p3, (self.long_size // 4, self.long_size // 4))
        p4 = self._upsample(p4, (self.long_size // 4, self.long_size // 4))
        p5 = self._upsample(p5, (self.long_size // 4, self.long_size // 4))
