...
parser.add_argument('--pre_ckpt_path', type=str, default='', help='Pretrain checkpoint file path')
...
if __name__ == '__main__':
    # load model
    pre_ckpt = args.pre_ckpt_path
    session = onnxruntime.InferenceSession(pre_ckpt, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
    random.seed(1024)
    t_d = 1792
    d = 550
    ...
    for data in tqdm(test_data_iter, '| feature extraction | test | %s |' % class_name,
        ...
        # model prediction
        inputs = {session.get_inputs()[0].name: np.expand_dims(data['img'][0].asnumpy(), axis=0)}
        outputs = session.run(None, inputs)
        # get intermediate layer outputs
        for j in range(len(outputs)):
            outputs[j] = Tensor(outputs[j])
    ...
    score_map = ops.interpolate(expand_dims(dist_list, 1), size=(224, 224)).squeeze().asnumpy()
