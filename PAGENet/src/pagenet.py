...
class upSampleLike(nn.Cell):
    ...
    def __init__(self):
        ...
        self.interpolate = P.interpolate

    def construct(self, fm, x1):
        fm = self.interpolate(fm, (x1.shape[2], x1.shape[3]))
        return fm
...
class MindsporeModel(nn.Cell):
    ...
    def mymax_16(self, target):
        ...
        target = resize(target, (target_shape[-2]/16, target_shape[-1]/16))
        return target

    def mymax_32(self, target):
        target_shape = target.shape
        resize = P.ResizeBilinearV2()
        target = resize(target, (target_shape[-2]/32, target_shape[-1]/32))
        return target
