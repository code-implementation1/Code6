...
from src.quantlib import QuantConv2d
...
def make_conv2d(in_channels, out_channels, kernel_size, a_bit=8, w_bit=8, has_bias=True):
    if in_channels == 3 or out_channels == 3:
        a_bit, w_bit = 8, 8

    return QuantConv2d(in_channels, out_channels, kernel_size=kernel_size,
    has_bias=has_bias, pad_mode="pad", padding=1, a_bit=a_bit, w_bit=w_bit)
